...
**What's verified vs. what isn't:** the title parser is tested against real post titles (above), and the full pipeline it feeds into — event ingestion, score increment, highlight attach, idempotency, ambiguous-team skip — is verified against real Postgres/Redis with the Reddit network call mocked (`tests/integration/reddit-live-poller.test.ts`). What's *not* verified is the live search itself: Reddit is unreachable from where this was built (network policy), so the exact flair string (`GOAL_CLIP_FLAIR` in `lib/reddit/live-detector.ts`) — the human-readable text without the emoji shortcode Reddit's UI shows next to it — hasn't been confirmed against a real search response. If it doesn't match, goal detection still works via the keyword fallback, just at lower precision.
